...
DEFAULT_CONFIG: Dict[str, Any] = {
    "code": {
        # 受控 A/B 的 A 臂（pre-governance）固定为该 commit；B/C/D 用当前代码
        "baseline_ref": "b4ae9ac",
    },
    "corpus": {
        # outputs/ 中的真实任务 job_id（When the Sky Was Ours）
        "job_id": "8126db91c3969845",
        # 第一轮先跑固定真实子集；[0, 0] 表示全文
        "subset": [0, 300],
    },
    "glossary": "",
    ...
    "run": {
        ...
        "enable_report": False,
        # 四臂共用同一批参数；temperature 由代码固定（translate 0.3 / repair 0.2 / review 0.2）
    },
    "arms": ["A", "B", "C", "D"],
    "seed": 42,
    "out_dir": "",
...
def describe_run_matrix(arms: List[str]) -> str:
    ...
    rows = {
        "A": ("pre-governance baseline（b4ae9ac）", "无", "无"),
        "B": ("current code + quality mode（冻结/画像/相关注入）", "有", "无"),
        "C": ("pre-governance baseline", "无", "有"),
