...
- A 臂通过 git worktree 固定到 b4ae9ac，与 B/C/D 共用同一模型/provider/
  temperature/批次参数/TM policy（TM 有无由种子控制）。

用法：
    python eval/run_ab.py --config eval/config.example.json [--mock]
    python eval/run_ab.py --config ... --segments 0:500
    python eval/run_ab.py --config ... --glossary-from-job 8126db91c3969845 --lock
"""
