...
与受控 A/B/C/D 分开，不进入同一 leaderboard。回答四类问题：
1. 真实书籍规模下历史任务暴露了什么问题；
2. 新系统是否覆盖已知 failure modes（对照回归测试）；
3. 3590 条真实 reviewed TM 能否被安全加载/利用；
4. 新 pipeline 能否在历史资产（state.json + source.bin）上正常工作。
