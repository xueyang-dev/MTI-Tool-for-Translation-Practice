...
- 🛡️ **断点续传和稳定性 (Robustness)**：任务进度实时落盘到本地 `outputs/` 目录，浏览器刷新、电脑重启后都能从断点继续；内置 API 防崩限流策略与重试机制。支持 OpenCode Go、DeepSeek、OpenAI、Gemini，模型可切换。
...
1. 在左侧边栏配置您的 API Key（默认 OpenCode Go / GLM-5.2），选择模型与翻译模式（快速 / 高质量）。
